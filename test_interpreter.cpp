...
TEST_CASE("Test Interpreter parser with expected input", "[interpreter]") {
    std::string program = "((r 10 define) (pi (r r *) *) begin)";
...
TEST_CASE("Test Interpreter special form: if", "[interpreter]") {
    {
        std::string program = "(True (4) (-4) if)";
        Expression result = run(program);
        REQUIRE(result == Expression(4.));
    }

    {
        std::string program = "(False (4) (-4) if)";
        Expression result = run(program);
        REQUIRE(result == Expression(-4.));
    }
}
...
TEST_CASE("Test Interpreter special forms: begin and define", "[interpreter]") {
    {
        std::string program = "(answer 42 define)";
        Expression result = run(program);
    ...
    {
        std::string program = "((answer (9 11 +) define) (answer) begin)";
        Expression result = run(program);
        REQUIRE(result == Expression(20.));
    }

    {
        std::string program = "((a 1 define) (b 1 define) (a b +) begin)";
        Expression result = run(program);
...
TEST_CASE("Test logical procedures", "[interpreter]") {
    REQUIRE(run("(True not)") == Expression(false));
    REQUIRE(run("(False not)") == Expression(true));

    REQUIRE(run("(True True and)") == Expression(true));
    REQUIRE(run("( True False and)") == Expression(false));
    REQUIRE(run("( False True and)") == Expression(false));
    REQUIRE(run("( False False and)") == Expression(false));
    REQUIRE(run("( True True False and)") == Expression(false));

    REQUIRE(run("( True True or)") == Expression(true));
    REQUIRE(run("( True False or)") == Expression(true));
    REQUIRE(run("( False True or)") == Expression(true));
    REQUIRE(run("( False False or)") == Expression(false));
    REQUIRE(run("( True True False or)") == Expression(true));
}

TEST_CASE("Test trig procedures", "[interpreter]") {
    REQUIRE(run("( pi sin)") == Expression(0.));
    REQUIRE(run("( pi cos)") == Expression(-1.));
    REQUIRE(run("( 1 0 arctan)") == Expression(atan2(1,0)));
}

TEST_CASE("Test some semantically invalid expresions", "[interpreter]") {
    std::vector<std::string> programs = {
        "(none @)", // no such procedure
        "(1 1 2 -)", // too many arguments
        "( if 1 define)", // redefine special form
        "( pi 3.14 define)"
...
TEST_CASE("Test graphic types", "[interpreter]") {
    ...
    {
        std::string program = "((0 0 point) (10 0 point) line)";
        std::istringstream iss(program);
        Interpreter interp;
    ...
    {
        std::string program = "((0 0 point) (10 0 point) pi arc)";
        std::istringstream iss(program);
        Interpreter interp;
    ...
    {
        std::string program = "((0 0 point) (10 10 point) rect)";
        std::istringstream iss(program);
        Interpreter interp;

        // expected expression:
        Expression exp;
        exp.head.type = RectType;
        exp.head.value.rect_value.point1.x = 0.;
        exp.head.value.rect_value.point1.y = 0.;
        exp.head.value.rect_value.point2.x = 10.;
        exp.head.value.rect_value.point2.y = 10.;

        REQUIRE(interp.parse(iss));
        REQUIRE(interp.eval() == exp);
    }

    {
        std::string program = "(((0 0 point) (10 10 point) rect) 200 100 50 fill_rect)";
        std::istringstream iss(program);
        Interpreter interp;

        // expected expression:
        Expression exp;
        exp.head.type = FillRectType;
        exp.head.value.fill_rect_value.rect.point1.x = 0.;
        exp.head.value.fill_rect_value.rect.point1.y = 0.;
        exp.head.value.fill_rect_value.rect.point2.x = 10.;
        exp.head.value.fill_rect_value.rect.point2.y = 10.;
        exp.head.value.fill_rect_value.r = 200;
        exp.head.value.fill_rect_value.g = 100;
        exp.head.value.fill_rect_value.b = 50;
    ...
    {
        std::string program = "(((0 0 point) (10 10 point) rect) ellipse)";
        std::istringstream iss(program);
        Interpreter interp;

        // expected expression:
        Expression exp;
        exp.head.type = EllipseType;
        exp.head.value.ellipse_value.rect.point1.x = 0;
        exp.head.value.ellipse_value.rect.point1.y = 0;
        exp.head.value.ellipse_value.rect.point2.x = 10;
        exp.head.value.ellipse_value.rect.point2.y = 10;
